...
class TpsGridGen(nn.Module):
    def __init__(self, out_h=256, out_w=192, use_regular_grid=True, grid_size=3):
...
def load_checkpoint(model, checkpoint_path):
    if not os.path.exists(checkpoint_path):
        return
    # Load checkpoint and move to the same device as the model
    ...
    if hasattr(model, 'device'):
        ...
        # If no device attribute, try to infer from model parameters
